 ...
 *
 * REQUEST-DEDUPED with React cache() (Phase 7): the slug layout and the page
 * beneath it both resolve the same prototype — the layout to inject brand
 * CSS custom properties before first paint, the page to render. cache()
 * makes those two calls ONE database round trip within a single request,
 * which is what keeps the "one query" property true now that two components
 * need the data instead of one.
 *
